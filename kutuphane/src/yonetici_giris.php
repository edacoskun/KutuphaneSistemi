<?php
require_once("baglanti_yonetici.php");                            
session_start();
...
<form action="yonetici_giris.php?step=basarili" method="POST">

    <center>
        <h2 id="white" class="h1-1">Yönetici Girişi</h2>
    ...
    <center>
        <a href="yonetici_anasayfa.html"><input type="submit" class="btn-1" name="login" value="Giriş" id="white" placeholder="submit"/></a>
    </center>
    </form>
    ...
    case "basarili":
        ...
        if(isset($_POST['login'])){
           ...
           if($mail == "" OR $sifre =="" OR $mail == " " OR $sifre == " "){
            ...
            header('Location : yonetici_giris.php?error=2');
            exit();
           ...
           else if($mail == "admin@hotmail.com" && $sifre == "admin123456")
           {
            ...
            if(filter_var($mail, FILTER_VALIDATE_EMAIL)){
                ...
                $calisanlar = mysqli_query($connection,"SELECT * FROM calisan WHERE calisan_email = '".$mail."' AND calisan_sifre = '".$sifre."'");
                $bul= mysqli_num_rows($calisanlar);

                if($bul > 0){
                    $query = mysqli_query($connection,"SELECT * FROM calisan WHERE calisan_email = '".$mail."' AND calisan_sifre = '".$sifre."'");

                    while($sonuc = mysqli_fetch_array($query)){

                        $_SESSION['calisan_id'] = $sonuc['calisan_id'];
                        $_SESSION['login'] = 5;
                ...
                header('Location : yonetici_giris.php?error=1');
                exit();
            }
        }
